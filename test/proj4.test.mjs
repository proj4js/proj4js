...
async function loadTestFile(filename) {
  ...
  return readFileSync(join(dirname(fileURLToPath(import.meta.url)), filename)).buffer;
}
...
describe('proj4', function () {
  ...
  it('axes should be invertable with proj4.transform()', function () {
    ...
    result = proj4(enu, wnu).forward({ x: 40, y: 50 }, true);
    assert.closeTo(result.x, -40, 0.000001);
    assert.closeTo(result.y, 50, 0.000001);
  });
  ...
  describe('Nadgrids BETA2007', function () {
    ...
    beforeAll(async function () {
      var buffer = await loadTestFile('BETA2007.gsb');
      initializeNadgrid(buffer);
    });
  ...
  describe('Nadgrids ntv2', function () {
    ...
    beforeAll(async function () {
      var buffer = await loadTestFile('ntv2_0_downsampled.gsb');
      initializeNadgrid(buffer);
    });
  ...
  describe('Nadgrids ntv2 no error columns', function () {
    ...
    beforeAll(async function () {
      var buffer = await loadTestFile('ntv2_0_downsampled_no_error_columns.gsb');
      initializeNadgrid(buffer);
    });
  ...
  describe('Nadgrids GeoTIFF with subgrids', function () {
    ...
    beforeAll(async function () {
      var buffer = await loadTestFile('ca_nrc_NA83SCRS.tif');
      var tiff = await fromArrayBuffer(buffer);
      await initializeNadgrid(tiff);
    });
