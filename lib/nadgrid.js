...
export function nadtifgrid(key, data, metadata) {
  metadata.ll = metadata.ll.map(d => degreesToRadians(d));
  metadata.del = metadata.del.map(d => degreesToRadians(d));

  const maxX = metadata.ll[0] + (metadata.lim[0] - 1) * metadata.del[0];
  const minY = metadata.ll[3] - (metadata.lim[1] - 1) * metadata.del[1];

  const latitudeOffsetBand = data[0];
  const longitudeOffsetBand = data[1];
  const nodes = [];

  for (let i = metadata.lim[1] - 1; i >= 0; i--) {
    for (let j = metadata.lim[0] - 1; j >= 0; j--) {
      const index = i * metadata.lim[0] + j;
